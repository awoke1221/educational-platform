...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: d1, error: e1 } = await db.from("Course").select("*");
    ...
    const {
      ...
      error: e2,
      count: c2,
    } = await db.from("Course").select("*", { count: "exact", head: false });
    ...
    const { data: d4, error: e4 } = await db.from("Lecture").select("*");
